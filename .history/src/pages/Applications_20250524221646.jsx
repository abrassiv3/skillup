...
const Applications = () => {
  ...
  const handleCreateChat = async (freelancerId) => {
            ...
            {applications.map((application) => (
              <li className="break-inside-avoid py-3" key={application.id}>
                <div className="flex flex-col gap-2 p-4 rounded-xl bg-neutral-700 text-white h-fit">
                  <div>
                    <p>
                      <strong>Project:</strong> {application.projectid.title}
                    </p>
                    <p>
                    ...
                    <p>
                      <strong>Freelancer:</strong> {application.freelancerid.firstname}{' '}
                      {application.freelancerid.lastname}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div
                      dangerouslySetInnerHTML={{
                        __html: application.proposal.replace(/\n/g, '<br/>'),
                      }}
                    ></div>
                  </div>
                  ...
                  <div>
                    <p
                      className={`font-bold py-0.5 w-fit px-2 bg-neutral-800 border rounded-2xl ${getStatusClass(
                        application.status
                      )}`}
                    >
                  ...
                  {application.projectid.accepting === true && (
                    <div className="flex gap-3 space-x-4 rounded-xl p-2 bg-neutral-800">
                      <button
                        ...
                        disabled={
                          application.status === 'APPROVED' || !application.projectid.accepting
                        }
                      >
                        Approve
                      </button>
                  ...
                  {application.status === 'APPROVED' && (
                    <div className="p-2 flex flex-col gap-1.5 text-center rounded-xl bg-neutral-800">
                      ...
                      <button
                        className="mt-2 bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-md"
                        onClick={() => handleCreateChat(application.freelancerid.userid)}
                      >
                        Create Chat
                      </button>
                    </div>
